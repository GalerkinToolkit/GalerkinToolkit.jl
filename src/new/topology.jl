...
function topology(geom::AbstractFaceDomain)
    D = num_dims(geom)
    if D != 0
        boundary = topology(GT.boundary(geom))
    else
        boundary = nothing
    end
...
num_dims(t::AbstractMeshTopology) = length(reference_topologies(t))-1
num_faces(t::AbstractMeshTopology,d) = length(face_reference_id(t,d))
...
function complexify(mesh::AbstractMesh;glue=Val(false))
    ...
    for d in (D-1):-1:0
        ...
        nrefid_to_ldface_to_lvertices = map(a->face_incidence(topology(complexify(domain(a))),d,0),newreffaces[n+1])
