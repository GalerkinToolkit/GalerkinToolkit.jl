...
function reference_face_boundary_from_reference_face(refface)
    ...
    ref_faces = map(ref_faces_geom) do ref_faces_geom_d
        map(r->lagrangian_reference_face(geometry(r);order=order_inter),ref_faces_geom_d)
    end
...
function unit_n_cube_boundary(
    ...
    if reference_face === nothing
        reference_face = lagrangian_reference_face(unit_n_cube(d-1))
    end
    my_boundary = if d == 1
...
function unit_simplex_boundary(
    ...
    if reference_face === nothing
        reference_face = lagrangian_reference_face(unit_simplex(Val(d-1)))
    end
    my_boundary = if d == 1
        node_coordinates = SVector{1,Float64}[(0,),(1,)]
        face_nodes = [[[1],[2]]]
...
function simplexify_unit_n_cube(geo)
    ...
    order = 1
    ref_cell = lagrangian_reference_face(simplex)
...
function reference_face_from_gmsh_eltype(eltype)
    ...
    lagrangian_reference_face(geom;order,lib_to_user_nodes=lib_to_gmsh)
end
...
function cartesian_chain(domain,cells_per_dir)
    ...
    ref_cell = lagrangian_reference_face(cell_geometry)
    reference_cells = [ref_cell]
...
function lagrangian_reference_face(
    ...
    init_coordinate=SVector{num_dims(geometry),Float64},
...
function partition_mesh(colorize,ranks,mesh;via=:nodes)
    if via === :nodes
        partition_mesh_via_nodes(colorize,ranks,mesh)
    elseif via === :cells
        partition_mesh_via_cells(colorize,ranks,mesh)
    else
        error("case not implemented")
    end
end
...
function partition_mesh_via_nodes(colorize,ranks,mesh)
    ...
    node_to_color = emit(colors_root;source=root)
...
function partition_mesh_via_cells(colorize,ranks,mesh)
    D = num_dims(mesh)
    ...
    colors_root = map(ranks) do rank
        if rank == root
            g = assemble_graph(ncells,[node_to_cells])
            colors::Vector{Int32} = colorize(g,length(ranks))
        else
            colors = Int32[]
        end
    ...
    pmesh = map(ranks,cell_to_color) do rank,cell_to_color
        ...
        lnode_to_colors = JaggedArray(view(node_to_cells,lnode_to_node))
        f = cell->cell_to_color[cell]
        lnode_to_colors.data .= f.(lnode_to_colors.data)
        lnode_to_colors = JaggedArray(map(colors->sort(unique(colors)),lnode_to_colors))
        lnode_to_owner = zeros(Int32,nlnodes)
        for (lnode, node) in enumerate(lnode_to_node)
            cells = node_to_cells[node]
            color = maximum(cell->cell_to_color[cell],cells)
            lnode_to_owner[lnode] = color
        end
        lface_to_face_mesh = map(face_nodes(mesh)[1:end-1]) do face_to_nodes
            nfaces = length(face_to_nodes)
            face_to_mask = fill(false,nfaces)
            for face in 1:nfaces
                nodes = face_to_nodes[face]
...
#        (;cldof_to_colors,cldof_to_owner,ncodofs,rank)
#    end
...
#        cldof_to_ldof = state.cldof_to_ldofs
